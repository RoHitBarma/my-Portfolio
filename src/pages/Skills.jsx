...
const Skills = ({theme}) => {
  return (
    <section className=" min-h-screen bg-gray-300 text-gray-800 dark:bg-gray-900 dark:text-white overflow-x-hidden">
      <div className='w-full max-w-6xl py-12 px-6 mx-auto'>
              ...
              <div className='w-20 h-20 sm:w-24 sm:h-24'>
                <CircularProgressbar
                  ...
                  styles={buildStyles({
                    ...
                    trailColor: theme === "dark" ? "#1f2937" : "#d1d5db",
                  })}
                />
              </div>
              <p className="text-lg font-semibold flex items-center gap-2">
                <span className="text-xl sm:text-2xl" style={{ color: skill.logoColor }}>{skill.icon}</span>
                {skill.name}
              </p>
            </motion.div>
          ))}
        ...
        >
          ...
          <div className="flex flex-wrap gap-3">
            {tools.map((tool, index) => (
              <span
                key={index}
                className="bg-gray-800 px-4 py-2 rounded-full text-sm text-gray-50 border border-gray-700 hover:border-indigo-500 transition"
              >
                {tool}
              </span>
            ))}
